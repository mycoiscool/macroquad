...
impl Camera {
    pub fn new(offset: Vec2<f32>, scale: f32) -> Self {
        Self {
            offset,
            scale,
    ...
    pub fn scale_wheel(&mut self, center: Vec2<f32>, wheel_value: f32, scale_factor: f32) {
        if wheel_value > 0. {
    ...
    pub fn scale_mul(&mut self, center: Vec2<f32>, mul_to_scale: f32) {
    ...
    pub fn scale_new(&mut self, center: Vec2<f32>, new_scale: f32) {
    ...
    pub fn update(&mut self, mouse_pos: Vec2<f32>, should_offset: bool) {
        if should_offset {
...
impl Into<Camera2D> for &Camera {
    fn into(self) -> Camera2D {
        ...
        Camera2D {
            zoom: Vec2::new(self.scale, -self.scale * aspect),
            offset: Vec2::new(self.offset.x, -self.offset.y),
            target: Vec2::new(0., 0.),
            rotation: 0.,
